...
So, back to the original ask, my go-to option was to recommend a Powershell script written by Steve Ottavi, which uses SSH to connect to the host(s) and figure all this out for you ([read about it here](/2020/05/27/how-to-count-dfw-rules-per-esxi-host/)), however, there was a constraint:
...
To see what else is possible when trying to log/send metrics from the host in this method of using a local shell script, have a read of one of my previous posts about [Monitoring DFW Heap Usage](/2017/06/06/monitoring-dfw-heap-usage/)
